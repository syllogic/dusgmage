...
class Syllogic_CustomProductImport_Adminhtml_CustomproductupdateimagesController extends Mage_Adminhtml_Controller_Action
{
            ...
            if(isset($_FILES['csv_import']['name']) && $_FILES['csv_import']['name'] != '')
				{
    				...
    				{       
        				...
        				$fname = "image_gallery.csv"; //file name                        
        				...
        				$filename = Mage::getBaseDir().DS.'magmi/integration/samples/uploads/update'.DS."image_gallery.csv";
...
$dp->beginImportSession("image_gallery_upload", "update", new TestLogger());
$filename =Mage::getBaseDir().DS.'magmi/integration/samples/uploads/update'.DS."image_gallery.csv";
$reader1 = new CSVReader( new FileReader( $filename ) );
...
$image_cell_no =  $product_attr_col_flip['image'];
$small_image_cell_no =  $product_attr_col_flip['small_image'];
$thumbnail_cell_no =  $product_attr_col_flip['thumbnail'];
$media_gallery_cell_no =  $product_attr_col_flip['media_gallery'];
...
if($rows != 1) // Skipping the headers row
{
   ...
   /* =========================Bulk Update of Stock ============================*/
             ...
             $item = array("sku"=>$cell[$sku_cell_no],'attribute_set'=>'Wears','type'=>'configurble','image'=>$cell[$image_cell_no],'small_image'=>$cell[$small_image_cell_no]
             ,'thumbnail'=>$cell[$thumbnail_cell_no],'media_gallery'=>$cell[$media_gallery_cell_no]);
   	           
                $dp->ingest($item);
                $message.="<li>".$cell[$sku_cell_no] ." Images successfully updated.</li>";
