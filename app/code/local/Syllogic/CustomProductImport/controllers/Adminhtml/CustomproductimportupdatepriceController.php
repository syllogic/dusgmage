...
class Syllogic_CustomProductImport_Adminhtml_CustomproductimportupdatepriceController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction()
    {
       ...
       $this->loadLayout();
       $this->_title($this->__("Bulk Update of Price "));
            ...
            if(isset($_FILES['csv_import']['name']) && $_FILES['csv_import']['name'] != '')
				{
    				...
    				{       
        				...
        				$filename = Mage::getBaseDir().DS.'magmi/integration/samples/uploads/update'.DS."price.csv";
...
if($rows != 1) // Skipping the headers row
{
   ...
   /* =========================Bulk Update of Stock ============================*/
             ...
             $item = array("sku"=>$cell[$sku_cell_no],'store' => $cell[$store_cell_no],
   	                   "price"=>$cell[$price_cell_no]);
   	           
                $dp->ingest($item);
                $message.="<li>".$cell[$sku_cell_no] ." price successfully updated to ".$cell[$price_cell_no]."</li>";
