...
class Syllogic_CustomProductImport_Adminhtml_CustomproductimportmetaController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction()
    {
       ...
       $this->loadLayout();
       $this->_title($this->__("Bulk Update of Product Meta  "));
            ...
            if(isset($_FILES['csv_import']['name']) && $_FILES['csv_import']['name'] != '')
				{
    				try
    				{       
        				$path = Mage::getBaseDir().DS.'magmi/integration/samples/uploads/product'.DS;  //desitnation directory     
        				$fname = "product.csv"; //file name                        
        				...
        				$filename = Mage::getBaseDir().DS.'magmi/integration/samples/uploads/product'.DS."product.csv";
...
$store_cell_no =  $product_attr_col_flip['store'];
$meta_title_cell_no =  $product_attr_col_flip['meta_title'];
$meta_keyword_cell_no =  $product_attr_col_flip['meta_keyword'];
...
if($rows != 1) // Skipping the headers row
{
   ...
   /* =========================Bulk Update of Stock ============================*/
             ...
             $item = array("sku"=>$cell[$sku_cell_no],
                            "store"=>$cell[$store_cell_no],
   	                   "meta_title"=>$cell[$meta_title_cell_no],
   	                   "meta_keyword"=>$cell[$meta_keyword_cell_no],
   	                   "meta_description"=>$cell[$meta_description_cell_no]);
   	           
                $dp->ingest($item);
                $message.="<li>".$cell[$sku_cell_no] ." meta title, meta keywords, meta description successfully updated </li>";
